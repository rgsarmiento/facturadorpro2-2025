...
class TenancyDatabaseSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // Solo insertar format_templates si la tabla está vacía
        $count = DB::table('format_templates')->count();
        if ($count === 0) {
        ...
        try {
            ...
            if (method_exists($this->command, 'info')) {
                $this->command->info('TipoComprobantesContablesSeeder completado');
            }
        } catch (\Throwable $e) {
            if (method_exists($this->command, 'warn')) {
                $this->command->warn('Advertencia: Falló TipoComprobantesContablesSeeder: '.$e->getMessage());
            }
        }
        ...
        try {
            ...
            if (method_exists($this->command, 'info')) {
                $this->command->info('UpdateDataServiceMasterTenantSeeder completado');
            }
        } catch (\Throwable $e) {
            if (method_exists($this->command, 'warn')) {
