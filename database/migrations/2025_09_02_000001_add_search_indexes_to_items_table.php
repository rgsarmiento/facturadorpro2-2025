...
class AddSearchIndexesToItemsTable extends Migration
{
    ...
    public function up()
    {
        Schema::table('items', function (Blueprint $table) {
            ...
            $table->index(['name', 'is_active'], 'items_name_active_index');
            $table->index(['internal_id', 'is_active'], 'items_internal_id_active_index');
            $table->index(['unit_type_id', 'is_active'], 'items_unit_type_active_index');
            $table->index(['is_active', 'is_set'], 'items_active_set_index');

            // Índice compuesto para las consultas más comunes
            $table->index(['is_active', 'is_set', 'unit_type_id', 'name'], 'items_search_composite_index');
        });
    }
